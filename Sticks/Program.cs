    ...
    class Program
    {
        static void Main(string[] args)
        {
            while (true)
            {
                ...
                WriteHowManyStickst(initialNumberOfSticks, starts);

                Game game = new Game(initialNumberOfSticks, starts);

                game.MachineMoved += (s, e) => Console.WriteLine($"Machine took {e.Taken} sticks. \n" +
                                                                $"Remains {e.Remains} sticks in the game.");
                game.GameOver += (s, player) => Console.WriteLine($"{player} won.");

                Polling(game);

                if (!RequestConfinuation())
                    break;
            }
        }
        ...
        private static void WriteHowManyStickst(int sticks, Player turn)
        {
        ...
        private static void Polling(Game g)
        {
            while (!g.IsGameOver())
            {
                ...
                else
                {
                    ...
                    WriteHowManyStickst(g.NumberOfSticks, g.Turn);
                }
            }
        }
        ...
        private static bool RequestConfinuation()
        {
            bool? result = null;
            while(result == null)
            {
